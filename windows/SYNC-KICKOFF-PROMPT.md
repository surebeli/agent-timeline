...
> 复测 `57 → 320` 且坐标不动。
>
> ### 命中区（第 7 项）：已检查并已放大到 22×26
>
> 实测四个头部图标按钮原为 **20×20 dip**（`WindowTool rect`，@100%）；mac 修完是 21×26pt
> —— Windows 竖向比 mac 修完后还小 23%，确是同一类「命中区约等于字形」的问题。
> 查证发现 `HeaderIconButtonStyle` **自初始提交 `bc2b497` 起从未改过**，`Padding="4"` 是原值，
> 被放大的只有 mac 那边。已对齐到 **22×26**。
>
> 做法是**给死 `Width`/`Height` 而不是加 Padding**：Padding 撑出来的尺寸随字形大小漂，
> 换个 Glyph 就变；固定尺寸才是稳定的命中框（mac 用的也是"固定 frame + contentShape"）。
>
> **原先预估的连锁没有发生**：头部行高由**带文字的过滤按钮**决定（实测 27dip），26 仍在其下，
> 故 HeaderBar 高度 43、折叠高度 57、三张 README 一览图全部不变（实测复验：折叠仍是 57，
> 运行时自检未报推导/实测不符）。背景是 Transparent，静止态零视觉变化。
>
