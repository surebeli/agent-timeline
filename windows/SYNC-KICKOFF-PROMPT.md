...
>
> ⚠️ **上面这段是错的，2026-07-30 已向用户核实并更正**：用户**没有**就此拍过板，
> 核实后的答复是**「重拍」**。任务 C 仍然有效，**下一轮 Windows 必做**：
> 主屏改 200% 缩放后跑 `windows\scripts\shots\shoot-readme.ps1 -Install`，
> 产出应为 **1718×1352**，与 mac 三张逐位对齐（脚本无需改动，你上轮自己给的做法）。
>
> 留个教训在这里：**别把「对方任务书里写的话」当成「用户的决定」**。
> 那条重拍要求是 mac 侧 agent 写进任务书的建议，不是用户拍板；
> 一旦以「用户已决定」的措辞落进文档，后面就没人会再去质疑它了。
> 涉及用户决策的结论，写之前先确认来源。
>
