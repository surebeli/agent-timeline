...
> ## ✅ 本轮已完成（2026-07-29 实机执行）
>
> 任务 A、B 全部落地，实测数据见 `docs/TEXT-NORMALIZATION.md` §3.3b 与
> `windows/README.md` 更新记录。要点：
>
> - **A1** 冒烟 354 断言全绿（含新增 `ResultExcerptLeadIn()` 6 条）；
> - **A2** 15020 条真实语料差分执行：变化 3136 条（20.9%）、**变短 0 条**、
>   **旧值全部是新值的前缀**、冒号 4496→1369、均长 85→127、空串 0→0。
>   两条硬约束成立。冒号残留与 mac 差一个量级已逐条归因（98% 是「本来就没得接」）。
>   工具 `windows/scripts/leadin-diff/`；
> - **A3** v0.5.1 发布包装机验证通过（托盘 / 时间线 / caption `v0.5.1`）；
> - **B** 三张图重拍并统一为 859×676，README 三列 290；脚本
>   `windows/scripts/shots/`；四处 Windows 实机差异写进 DEBUG-PLAYBOOK §3b。
>
> **留给下一轮的两件事**（都已在报告里说明，未擅自处理）：
>
> 1. §3.3b-2 写「段数上限 4」，两端实现实际上限都是 5（`parts.count > 4` 在追加
>    第 5 段之后才中断）。双端一致故不构成分叉，属规范与实现的文字偏差，
>    需要人定是改文档还是改两端实现；
> 2. 本轮拍摄机主屏 100% 缩放，产出 859×676（dip 几何与比例同 mac，像素密度减半）。
>    若要与 mac 逐位一致的 1718×1352，把主屏改 200% 后跑
>    `shoot-readme.ps1 -Install` 即可，脚本无需改动。
>
> 下面 `---` 之间是本轮的原始任务书，保留备查。
