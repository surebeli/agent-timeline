...
| 渲染缩放 | **按拍摄机系统默认，脚本不改缩放** | 用户 2026-07-30 定：各端用自己系统的默认设置，不再互相强制。mac 为 Retina 2x、Windows 为该机 100%——**dip 几何与宽高比两端相同**，README 三列同宽时仍严丝合缝，只是像素密度不同 |
| 合成画布 | 三态并集 + 96px 四边留白（按 dip 折算） | mac 得 **1718 × 1352**，Windows 100% 得 **859 × 676**。投影/圆角按画布宽折算，不写死像素 |
...
**缩放**：`-Scale` 默认 200，但**成品定的是 100%**（见下）。当前缩放与 `-Scale` 不符时
脚本**自己改**（`WindowTool scale set`，走系统「设置」同一条 DisplayConfig 路径，立即生效
不用注销），改完不改回。要改回自己跑 `WindowTool scale set 100`。改的是全局缩放，
会重排屏幕上所有已打开的窗口。

改不动时会停下来报工具原话——常见原因是主显示器不在 Windows 显示配置库（CCD）里，
远程会话 / 虚拟显示适配器驱动的桌面就是这样（2026-07-30 本机实测：`GameViewer Virtual
Display Adapter` 下三种 flag 的活动路径都是 0，`PerMonitorSettings` 键不存在，
`SPI_SETLOGICALDPIOVERRIDE` 每档返回 False；换回物理屏直连后同一条路径立刻可用）。

**成品用 100%，不是 200%**（2026-07-30 用户定：「恢复到 100%，之前有记录 200% 应该是
错误的」）。产出 859×676——画布**宽高比与 dip 几何和 mac 的 1718×1352 逐位相同**
（859×676 就是同一组 dip 常量），README 三列同为 290 时两行仍严丝合缝对齐，只是像素
密度为 mac 的一半。投影位移/模糊与圆角半径都按 `画布宽 / 859dip` 折算、不是写死像素，
所以将来若真要换密度重拍，代码无需改动。

拍摄前 `-Scale` 请显式传 100；那条「重拍到 200%」的历史要求已作废，来源链见
`windows/SYNC-KICKOFF-PROMPT.md` 顶部。
