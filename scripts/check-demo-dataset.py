#!/usr/bin/env python3
"""演示数据集的四语不变式校验（docs/DEMO-DATASET.md）。

守两条：
  1. **结构四语一致**——agent / 项目 / session / 时间戳 / kind / 代号生命周期完全相同。
     结构一旦分叉，各语的版面与折行就不同，README 中英两行（目前仅 zh/en 上 README）对不齐；
  2. **文案两两不同**——命令原文 / 标题 / 关键点 / 结果行 / 代号定义都换了语言。
     任意一对语言全等说明其中一套忘了翻，而那种图看着完全正常（chrome 是对的语言、内容不是）。
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        win = {lang: seed(tmp, lang, end="win") for lang in LANGS}
        mac = {lang: seed(tmp, lang, end="mac") for lang in LANGS}
    ...
    # ── 结构跨语言一致：以 zh 为基准，其余语种的 agent/项目/session/ts/kind/代号生命周期
    # 必须逐位相同——结构一旦分叉，各语版面与折行就不同，README 对应行就对不齐了。
    ref = win["zh"]
    for lang in LANGS[1:]:
        data = win[lang]
        if data["struct"] != ref["struct"]:
            fails.append(f"结构（agent/项目/session/ts/kind）{lang} 与 zh 不一致——版面会对不齐")
        if data["codes"] != ref["codes"]:
            fails.append(f"代号生命周期（定义节点/次数/状态/状态节点/更新时间）{lang} 与 zh 不一致")
        if data["embedded"] != ref["embedded"]:
            fails.append(f"节点内嵌 codenames 的 name/status {lang} 与 zh 不一致")

    # ── 文案两两不同：任意一对语言若文案全等，说明其中一套没翻，会拍出混语图。
    for a, b in itertools.combinations(LANGS, 2):
        da, db = win[a], win[b]
        if da["texts"] == db["texts"]:
            fails.append(f"{a}/{b}: 文案完全相同——有一套没翻，会拍出混语图")
        else:
            same = [i for i, (x, y) in enumerate(zip(da["texts"], db["texts"])) if x == y]
            if same:
                fails.append(f"{a}/{b}: 第 {[i + 1 for i in same]} 条的文案相同——漏译")
        if da["defs"] == db["defs"]:
            fails.append(f"{a}/{b}: 代号定义完全相同——漏译")
    ...
    print(f"演示数据集校验通过：{len(ref['struct'])} 节点 × {len(LANGS)} 语种 × 2 端，"
