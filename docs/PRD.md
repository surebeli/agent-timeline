...
- **自稳对比（任意底色）**：半透明面板叠在任意明暗窗口（暗色 IDE/terminal 为常态）上仍需可读——① `panelScrim` 底幕层垫在模糊材质与内容之间，压缩透入底色方差且保持透光；② 两级纸面附 1px `surfaceStroke` 自适应描边，块面在同色系底上自带边界。不牺牲透明特征。
...
### 3.3b 类型锚点（F3b，2026-07-26 新增；原名「阶段锚点」，2026-07-29 更名）
