...
7. 空行折叠（≥3 个 `\n` → 2 个）→ 交给 `ResultExcerpt` 取首段（+ 引子续接，见 §3.3b）→ `Clip`
...
### 3.3b 引子续接（2026-07-29 落地，双端同步）

**问题**：`ResultExcerpt` 取「首个非空段落」，而 agent 回复极常见的形态是
**引子 + 空行 + 正文**——首段只是一句以冒号收尾的过渡，真正的内容在下一段。
结果行于是只剩引子。用户上报的实例：

```
TH-0025 是一条**安全**类 issue,核心是一句话:      ← 首段（引子），结果行只到这里
                                              ← 空行
> **harnessloop 让 agent「绝不要把凭证写进…**    ← 正文，整段丢失
```

**规则**：首段去尾空白后以 `:` 或 `：` 收尾时判为「引子」，继续吃下一段，
直到吃到第一个非引子段为止；段间以**单个空格**拼接，仍受 `Clip(maxLength)` 约束。

硬性约束（双端实现必须满足，已各自断言）：

1. **首段一字不动**：只对**被续接进来**的段落剥行首 `> ` / `- ` 标记
   （规整层在 Excerpt 档只剥全文首行，见 §3.3 引用/列表规则）。非引子回复的产出
   与本规则落地前**逐字节一致**；
2. **续接上限 4 段**（首段之外；含首段合计最多 5 段）：真实语料里引子链最多两层，
   给到 4 段是防病态输入的兜底。⚠️ 2026-07-30 订正——原写「段数上限 4」，
   而两端实现（`parts.count > 4` 在**追加第 5 段之后**才中断）合计上限都是 5。
   双端一致故不构成分叉，是规范的文字偏差；**改文档不改代码**，这个数字本就是任意
   安全界，动代码要重跑两端差分验证（mac 2460 条 / win 15020 条）不值当
   （与 §3.4-2「凑够即停」的扫描预算同源）；累计长度达 `maxLength` 也停；
3. **末级兜底不续接**：兜底链最后一级走**未规整**原文（围栏/表格都还在），
   续接会把表格行拼进来，故只取首段——§3.4-1 在那一级只要求「不为空」；
4. **切段按空行**：规整层把表格/围栏整块删掉却保留两侧空行，故连续空行只算一个分隔。

**实测**——差分执行：同一份**冻结**语料，用改动前 / 后两个源码状态各跑一遍
`ResultExcerpt`，逐条比对产出。

| 指标 | mac（2460 条真实 agent 回复） | Windows（15020 条，2026-07-29 复现） |
|---|---|---|
| 产出变化 | 350 条（14.2%） | 3136 条（20.9%） |
| 变短（回归） | **0 条** | **0 条** |
| 变化形态 | **旧值全部是新值的前缀**——只续接、不改写 | **旧值全部是新值的前缀** |
| 冒号结尾 | 351 条 → **2 条**（余下 2 条正文全在代码围栏里，Excerpt 档按规范丢弃，无散文可接） | 4496 条 → **1369 条**（归因见下） |
| 平均长度 | 90 → 114 字符 | 85 → 127 字符 |
| 空串 | 0 → 0（§3.4-1 不变式保持） | 0 → 0 |

「变短 0 条」与「旧值是新值的前缀」两端都成立——从数据上证明本改动只可能加内容、
不可能改内容。

Windows 语料 = `%USERPROFILE%\.claude\projects` 的全部 assistant 回复 +
`%USERPROFILE%\.codex\sessions` 的全部 `payload.last_agent_message`，经**真实解析器**
...
| 桶 | 条数 | 说明 |
|---|---|---|
| 回复只有一段 | 1341 | 引子后根本没有下一段，规则无从续接 |
| 后续段被 Excerpt 档丢弃 | 19 | 正文全在围栏/表格里（mac 剩下的那 2 条即此类） |
| 续接后末段仍冒号收尾 | 9 | 引子链吃到段数/长度上限，兜底生效 |
| 未归类 | 0 | |

98% 落在「本来就没得接」：Windows 这台机器的语料里，agent 以
「…开跑:」「…先确认:」收尾紧接工具调用的工作流水远多于 mac 语料。
**不是实现少接了**，规则已尽其所能。

**不回填**：已入库的结果行不重算（用户 2026-07-29 决定）。本机库 357 条结果行中
14 条是该形态，保持原样；规则只对新节点生效。
...
（2026-07-29 起首段是引子时会续接下一段，见 §3.3b，但仍止于「开头若干段」），
...
### 3.6 匹配态兼容折叠（2026-07-30 落地，双端同步）

规整层此前只服务**展示**。识别词表四语化时暴露出一个独立需求：**关键词子串匹配**
需要另一个更窄的归一档——日语全角英数 `ＷＩＰ`、半角片假名 `ﾃﾞﾌﾟﾛｲ`、分离浊点会让
子串匹配整个失效，而这些形态在日语语料里很常见。

入口：`TextNormalizer.FoldForMatch` / `ForMatch`（后者叠加小写）。
使用方：`CodenameDetector` 状态词表、`RuleSummarizer` 类型词表。**不作用于任何展示文本**。

**为什么不用平台 NFKC**（这是本节的关键结论，双端都不要"优化"回去）：

1. `String.Normalize(FormKC)` 在 `InvariantGlobalization=true` 下**静默原样返回**——
   不抛异常。`CoreSmokeTest` 正是这个配置、主程序不是，照搬会让「门禁跑的语义」与
   「线上跑的语义」不同，且是无声的。这个坑是本轮实测踩出来的；
2. 平台 NFKC 各自绑 ICU 版本，macOS 与 Windows 的 ICU 并不同版。双端对齐靠
   「各自调各自的 NFKC」反而不牢，靠一张写死的表才逐字节可复现。

因此 mac 侧**不要**改用 `precomposedStringWithCompatibilityMapping`，照搬同一张表。

覆盖面按真实需要划定：表意空格 `U+3000`、全角英数记号 `U+FF01–FF5E`、
半角片假名 `U+FF61–FF9F`、浊点/半浊点合成（`U+3099/309A`、`U+309B/309C`、半角 `ﾞ/ﾟ`）。
圈号数字、合字、上下标这些 NFKC 也管的，与关键词匹配无关，不做。

**配套的两条匹配判据**（同样双端同源）：

- **拉丁词必须落在词边界上**，CJK 词不设边界（`N2完成` / `バグ修正` 是自然写法）。
  不加这条，纯子串匹配会误命中一批极常见的词：`prefix`/`suffix` 含 fix、
  `networking` 含 working、`disclosed` 含 closed、`swipe` 含 wip；
- **中日同形词是词表最大的坑**：日语词若与简体中文写法相同，就等于同时加进了中文表。
  5189 条真实命令上量过，`要求`（→需求）、`判断`（→决策）必须排除——它们在中文里是
  高频通用动词，而这两档都判在 Task 之前。日语侧改用无碰撞的 `要件/仕様`、
  `決定/選定/方針`，覆盖不受影响。反过来 `調査/検討/説明/実装/対応` 用的是日本新字体，
  简体写法不同，安全。
...
- **B1 codex `session_meta` 取值不一致**：`EnsureMeta`（重启续扫）只读**第 0 行**，
  ...
  **mac 侧评估（2026-07-28，按任务书要求先评估后落地）**：
  - **实证**：本机 261 个 rollout 中 55 个在第 0 行之后仍有 session_meta（44 个 id 不同）。
    ⚠️ **更正（2026-07-28 复核）**：此前记「用户库里 38 组 / 41 行重复节点」系判据错误——
    只比了「同 `source_file`+同正文」，把用户在不同时刻的重复输入（「继续」10 次、
    「需要」3 次…）算成了缺陷产物。mac 的 `nodes` 表**没有 `source_offset` 列**，
    改用严格判据（同文件+同正文+**同时间戳**+不同 `session_id`，即同一物理行被解析两次）
    实测 **0 组**。原因：mac 不重扫已消费字节（偏移持久化 + `makeContext` 只重读首行），
    故 B1 在 mac 端只表现为**结果行跨文件错配**，不产生重复行；Windows 的 257 组 / 514 行
    是该端 `EnsureMeta` + 重扫路径的特有表现，判据是同 `source_file`+同 `source_offset`。
